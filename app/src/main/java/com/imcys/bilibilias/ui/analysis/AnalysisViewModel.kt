...
class AnalysisViewModel(
     ...
     * 解析输入管线（防抖 ＋ **不做等值去重**）。
     *
     * 原先这里是 `MutableStateFlow<String?>` ＋ `debounce` ＋ `distinctUntilChanged()`：
     * StateFlow 对等值不发射，于是**同一段文本再提交一次毫无反应** ——
     * "同一个链接再粘一次"看不出变化，错误卡片的「重试」也无从实现。
     * 管线抽到 `:core:data`，那条性质有单测钉着（`AnalysisInputPipelineTest`）。
     */
    private val inputPipeline = AnalysisInputPipeline(debounceMillis = debounceTime)
     ...
     * 错误卡片上的「重试」。
     *
     * 走的是和输入框**同一条**管线：因为管线不做等值去重，重试同一段文本是真的会重新解析的
     * （改造前用 `MutableStateFlow` 时，重试同一段文本不会有任何反应）。
     */
    fun retryParse() {
        val text = _uiState.value.inputAsText
        if (text.isBlank()) return
        _uiState.value = _uiState.value.copy(parseErrorMessage = null)
        inputPipeline.submit(text)
    }

    /**
     * 记录一次"解析没能产出结果"的原因。
     *
     * 第十八轮审查里，五条失败路径全是 `else -> {}` / `onFailure {}` / `null -> {}`：
     * 页面就停在空白上，用户分不清"没识别出来"、"被风控"还是"App 卡死了"。
     * 现在每条失败路径都必须经过这里，界面上会显示一张带「重试」的错误卡片。
     */
        ...
        videoInfoRepository.getDonghuaSeasonViewInfo(seasonId = seasonId).collect {
            when (it.status) {
                ApiStatus.SUCCESS -> {
                    ...
                    // ⚠️ 这里以前写的是 `section[0].episodes[0]`：**section 非空不代表它下面有剧集**，
                    // 后者为空时 IndexOutOfBoundsException 会一路直达 AppCrashHandler。
                    // 分支条件保持与原写法一致，只把下标访问换成 null 安全的取值。
                    val firstEpId = if (it.data?.episodes?.isEmpty() != true) {
                        it.data?.episodes?.firstOrNull()?.epId ?: 0L
                    } else if (it.data?.section?.isEmpty() != true) {
                        it.data?.section?.firstOrNull()?.episodes?.firstOrNull()?.epId ?: 0L
                    } else {
                        -1L
                    }

                    _uiState.value = _uiState.value.copy(
                        asLinkResultType = ASLinkResultType.BILI.Donghua(firstEpId, it)
                    )
...
//                }
    ...
    private suspend fun handleUserSpace(text: String) {
        // 安全解析：uid 来自用户复制的文本，超长数字会让 toLong() 抛 NumberFormatException，
        ...
        videoInfoRepository.getVideoView(aid = aid, bvId = bvId).collect {
            ...
            when (it.status) {
                ApiStatus.SUCCESS -> {
                    ...
                    // 注：原先这里会去原作者服务器查「充电专属 BoostVideo」并驱动下载页的显示/拦截。
                    // 该接口随域名一起失效（且会因数据取不到而恒为 null，导致 isUpowerExclusive
                    // 的视频下载区永远不渲染），已整条移除；是否可播放改由 B 站自身的
                    // isUpowerPlay 字段判断。
