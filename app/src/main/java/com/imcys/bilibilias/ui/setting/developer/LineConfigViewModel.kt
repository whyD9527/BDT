    ...
    private suspend fun speedTest(lineHost: BILILineHostItem, rawUrl: String) = try {
        ...
        withContext(Dispatchers.IO) {
            ...
            withTimeout(5000) {
                ...
                // 加UA
                // 请求头规则统一从 RequestRules 取（这里走的是 java.net 裸连接，
                // 不是 Ktor，所以不能复用插件，但值必须是同一份）
                val isTv = platformType.isTv
                connection.setRequestProperty("User-Agent", RequestRules.speedTestUserAgent(isTv))
                RequestRules.speedTestReferer(isTv)?.let { referer ->
